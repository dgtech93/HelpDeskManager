...
File tipico: **`HelpDesk Manager_0.4.0_x64-setup.exe`** (nome legato a `version` in `package.json` / `src-tauri/tauri.conf.json`).
...
4. Crea la release GitHub e allega l’installer (percorso con spazi tra virgolette):
   ```powershell
   gh release create v0.4.0 --title "HelpDesk Manager 0.4.0" --generate-notes "src-tauri/target/release/bundle/nsis/HelpDesk Manager_0.4.0_x64-setup.exe"
   ```
