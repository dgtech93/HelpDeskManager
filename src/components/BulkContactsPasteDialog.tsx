...
import { parseContactPasteGrid, type ParsedContactPasteRow } from "@/lib/bulkPasteContacts";
import {
...
export function BulkContactsPasteDialog({ open, onClose, clients, lockedClientId, onImported }: Props) {
  ...
  const parsed = useMemo<ParsedContactPasteRow[]>(
    () => (open ? parseContactPasteGrid(raw, delimiter, clients, lockedClientId) : []),
    [open, raw, delimiter, clients, lockedClientId],
  );
  ...
  const maxCols = useMemo(() => {
    let m = lockedClientId ? 6 : 7;
    for (const r of rowsForPreview) {
      if (r.kind === "error") m = Math.max(m, r.cells.length);
    }
    return m;
  }, [rowsForPreview, lockedClientId]);

  if (!open) return null;

  const columnTitles = lockedClientId
    ? ["Nome", "Cognome", "Ruolo", "Email", "Telefono", "Cellulare"]
    : ["Cliente", "Nome", "Cognome", "Ruolo", "Email", "Telefono", "Cellulare"];

  const verifyAndImport = async () => {
    const toCreate = parsed.filter(
      (r): r is { kind: "ok"; sourceLine: number; payload: CreateClientContactInput } => r.kind === "ok",
    );
    ...
    try {
      ...
      for (const row of toCreate) {
        await api.createContact(row.payload);
        ok++;
      }
      const msg = `${ok === 1 ? "1 contatto creato" : `${ok} contatti creati`}${
        errCount > 0
    ...
    <div className="fixed inset-0 z-[60] overflow-y-auto bg-black/45 p-4 backdrop-blur-sm">
      <div className="mx-auto mt-6 flex max-h-[calc(100vh-3rem)] w-full max-w-5xl flex-col rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-950">
        <div className="flex items-start justify-between gap-3 border-b border-slate-200 px-5 py-4 dark:border-slate-700">
          <div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-50">Inserimento multiplo contatti</h2>
            <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">
              Ogni <strong>riga</strong> è un referente. Ordine colonne: {!lockedClientId ? (
                <>
                  <strong>Cliente</strong> (nome come in Clienti), <strong>Nome</strong>, <strong>Cognome</strong>,
                </>
              ) : (
                <>
                  <strong>Nome</strong>, <strong>Cognome</strong>,
                </>
              )}{" "}
              <strong>Ruolo</strong>, Email, Telefono, Cellulare. Serve almeno nome o cognome.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="flex-shrink-0 space-y-3 border-b border-slate-100 px-5 py-3 dark:border-slate-800">
          ...
          <p className="text-[11px] text-slate-500 dark:text-slate-500">
            Le righe sono separate da a capo. Verifica sempre l&apos;anteprima prima di confermare con la master password.
          </p>
        </div>
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-hidden px-5 py-3">
          ...
          <textarea
            ...
            placeholder={
              lockedClientId
                ? "Es.: Mario\tRossi\tIT\tmario@...\nLaura\tBianchi\t..."
                : 'Es.: Acme Srl\tMario\tRossi\tIT\t...\nBeta SpA\tLaura\t...\n'
            }
                ...
                <thead className="sticky top-0 z-[1] border-b border-slate-200 bg-slate-100 font-semibold text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
                  <tr>
                    ...
                    {Array.from({ length: maxCols }, (_, i) => (
                      <th
                        key={i}
                        className="min-w-[6rem] border-r border-slate-200 px-2 py-2 last:border-r-0 dark:border-slate-700"
                      >
                        {columnTitles[i] ?? `Col ${i + 1}`}
                      </th>
                    ))}
